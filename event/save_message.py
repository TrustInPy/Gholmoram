...
async def handler(event):
    ...
    if event.message.forward:
        if type(event.message.forward.from_id) is PeerUser:
            forward_from_user_id = event.message.forward.from_id.user_id
        elif type(event.message.forward.from_id) is PeerChannel:
            forward_from_user_id = event.message.forward.from_id.channel_id
        else:
            forward_from_user_id = "Unknown"
    else:
    ...
    if event.photo:
        ...
        if event.message.message:
            message_text = (
                f"<Document ({event.document.mime_type})>\n {event.message.message}"
            )
        else:
